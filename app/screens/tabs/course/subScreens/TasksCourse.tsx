...
export default function TasksCourse() {
  const [isPressed, setIsPressed] = useState(false);
  const list = ['Bandeja de entrada', 'No leído', 'Destacado']
  return (
    <View className='bg-light-primary flex-1' >
      <View className='p-3 gap-3'>
        <Text className='text-2xl font-semibold'>Total de las tareas</Text>
        <Text className='text-justify'>Se listarán todas las tareas, las que suman para tu promedio final se indicarán como calificadas y las que no se indicarán como no calificadas.</Text>
      ...
      <View className='px-3 flex-1 bg-light-primary'>
        <ScrollView className='border-t-2 py-3'>
          ...
          >
            ...
            <Text>Semana 1</Text>
            <View className='flex-row gap-2 item border-1 bg-light-true border-[1px] py-1 px-2 items-center self-start'>
              <Icon name="check" size={16} color={Colors.light.primary} />
              <Text className='text-light-primary'>Entregado</Text>
            </View>
            <View>
